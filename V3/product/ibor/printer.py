...
def portfolio_response(response):
    ...
    if isinstance(response, lm.PortfolioDetails):
...
def upsert_quotes_response(response):
    ...
    for quote_correlation_id, quote in response.values.items():
        row_data = quote.model_dump()
        quote_series = quote.quote_id.quote_series_id.model_dump()
        metric = quote.metric_value.model_dump()
        row_data.update(quote_series)
        row_data.update(metric)
...
def get_holdings_df(response):
    ...
    for holding in response.values:
        current_row = {}
        current_row.update(holding.model_dump())
        for field in nested_fields:
            del current_row[field]
