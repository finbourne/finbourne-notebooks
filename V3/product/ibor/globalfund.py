...
def valuation(api_factory, marketdata_scope, portfolio_group, time):
    ...
    # Perform a valuation
    response = api_factory.build(lu.AggregationApi).get_valuation(
...
def create_portfolio_group(api_factory, scope, code, portfolios):
    ...
    try:
        api_factory.build(lu.PortfolioGroupsApi).delete_portfolio_group(
...
def create_portfolios(api_factory, scopes, code, currency):
    ...
    for scope in scopes:
        ...
        try:
            api_factory.build(lu.PortfoliosApi).delete_portfolio(
                scope=scope, code=code
            )
...
def create_cut_labels(api_factory, exchange_names, cut_label_type):
    ...
    for exchange in exchange_names:
        ...
        exchange_time = models.CutLocalTime(
            hours=exchange_info[exchange]["hour"], minutes=exchange_info[exchange]["minutes"]
        )
        request = models.CreateCutLabelDefinitionRequest(
            code=exchange_code,
